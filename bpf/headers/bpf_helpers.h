...
#define SEC(name) \
	_Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wignored-attributes\"") __attribute__((section(name), used)) \
...
#define ___bpf_pick_printk(...) \
	___bpf_nth(_, ##__VA_ARGS__, __bpf_vprintk, __bpf_vprintk, __bpf_vprintk, __bpf_vprintk, __bpf_vprintk, __bpf_vprintk, __bpf_vprintk, \
		__bpf_vprintk, __bpf_vprintk, __bpf_printk /*3*/, __bpf_printk /*2*/, __bpf_printk /*1*/, __bpf_printk /*0*/)
...
#define bpf_for_each(type, cur, args...) \
	for (/* initialize and define destructor */ \
		...
		*___p __attribute__((unused)) = \
			(bpf_iter_##type##_new( \
				 &___it, ##args), /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_##type##_destroy() when \
																											 used from cleanup() attribute */ \
...
#define bpf_for(i, start, end) \
	for (/* initialize and define destructor */ \
		...
		*___p __attribute__((unused)) = \
			(bpf_iter_num_new(&___it, (start), \
				 (end)), /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_num_destroy() when used from \
																									cleanup() attribute */ \
...
#define bpf_repeat(N) \
	for (/* initialize and define destructor */ \
		...
		*___p __attribute__((unused)) = \
			(bpf_iter_num_new( \
				 &___it, 0, (N)), /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_num_destroy() when used \
																											 from cleanup() attribute */ \
				(void)bpf_iter_num_destroy, (void *)0); \
